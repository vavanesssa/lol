...
app.post("/api/removelivetoteam", async (req, res) => {
  const { id } = req.body;
  try {
    const foundTeam = await Team.findOne({ id });
    let updatedPlayers;
    if (foundTeam) {
      const players = await Player.find({ id: { $in: foundTeam.playersInTeam } });
      const writeOperations = players.map((player) => {
        if (player.lives > 0) {
          player.lives -= 1;
        }
        return {
          updateOne: {
            filter: { id: player.id },
            update: { lives: player.lives }
          }
        };
      });
      updatedPlayers = await Player.bulkWrite(writeOperations);
    }
    res.json(updatedPlayers);
    logger(`POST / Remove Live to Players Team ${JSON.stringify(updatedPlayers, null, 4)} `);
  } catch (err) {
    logger(`Error: ${err} `);
    return res.status(500).json({ error: "Internal Server Error" });
  }
...
app.post("/api/addlivetoteam", async (req, res) => {
  const { id } = req.body;
  try {
    const foundTeam = await Team.findOne({ id });
    const settings = await GameSettings.findOne();
    ...
    res.json(updatedPlayers);
    logger(`POST / Add Live to Players Team ${JSON.stringify(updatedPlayers, null, 4)} `);
  } catch (err) {
    logger(`Error: ${err} `);
    return res.status(500).json({ error: "Internal Server Error" });
  }
